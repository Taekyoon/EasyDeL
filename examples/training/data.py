    ...
    def __iter__(self):
        ...
        for tokens, loss_masks, loc, index in self.parallel_example_iterator():
            ...
            while len(token_buffer) > chunk_size + 1:
                ...
                batch = {
                    'input_ids': np.array(token_buffer[:chunk_size], dtype=np.int32).reshape(-1),
                    # 'target_tokens': np.array(token_buffer[1:chunk_size + 1], dtype=np.int32).reshape(-1),
                    # 'loss_masks': np.array(loss_mask_buffer[1:chunk_size + 1], dtype=np.float32).reshape(-1),
                    'attention_mask': np.array(loss_mask_buffer[:chunk_size], dtype=np.int32).reshape(-1),
                }
                if self.config.always_start_with_bos:
                    batch['input_ids'][0] = self.tokenizer.bos_token_id
                # yield batch, metrics
                yield batch
                token_buffer = token_buffer[chunk_size:]
                loss_mask_buffer = loss_mask_buffer[chunk_size:]
